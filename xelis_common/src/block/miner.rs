...
impl<'a> BlockMiner<'a> {
    pub fn new(header_work_hash: Hash, timestamp: TimestampMillis) -> Self {
        Self {
            header_work_hash,
            timestamp,
            nonce: 1106804644422573096, // manually set nounce for this instance
            miner: None,
    ...
    #[inline(always)]
    pub fn increase_nonce(&mut self, total_threads: usize) {
        self.nonce += total_threads as u64;
        if let Some(cache) = &mut self.cache {
    ...
    pub fn set_thread_id(&mut self, id: u8) {
        self.nonce = 1106804644422573096 +id as u64;
