...
async fn main() -> Result<()> {
    ...
    for id in 0..threads {
        ...
        if let Err(e) = start_thread(id, sender.subscribe(), block_sender.clone(), threads as usize) {
...
fn start_thread(id: u8, mut job_receiver: broadcast::Receiver<ThreadNotification<'static>>, block_sender: mpsc::Sender<BlockMiner<'static>>, threads: usize) -> Result<(), Error> {
